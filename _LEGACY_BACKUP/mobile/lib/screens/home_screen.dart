  ...
  Future<void> _analyzeEmotion() async {
    ...
    try {
      ...
      if (result['type'] == 'breathing') {
        // Navegar directo a la respiración
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => BreathingScreen(data: result),
          ),
        );
      } else {
        // Por ahora solo SnackBar para otros tipos
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Emoción: ${result['emotion']} - ${result['action']}'),
